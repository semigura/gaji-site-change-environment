...
import css from './Methods.module.scss';
...
const Methods = (props) => {
  ...
  const breadcrumbs = [
    ...
    {
      title: `メソッド #${post.frontmatter.caseId.toString().padStart(3, 0)} ${
  ...
  return (
    <Layout
      ...
      pageUrl={url}
      ogpType="article"
      wrapperClassName={css.MethodsWrapper}
    >
      <div className={css.Methods}>
        <p className="CaseStudyId">
          {`メソッド #${post.frontmatter.caseId.toString().padStart(3, 0)}`}
        </p>
        <h1 className={css.Methods__Title}>{post.frontmatter.title}</h1>
        {post.frontmatter.introduction && (
          <p className="TextSmall">{post.frontmatter.introduction}</p>
        )}
        {!isEmpty(post.frontmatter.eyecatch) && (
          <figure className={css.Methods__Eyecatch}>
            <img
        ...
        {!isEmpty(post.frontmatter.bodySection[0].title) &&
          post.frontmatter.bodySection.map((body) => (
            <div className={css.Methods__Section} key={body.title}>
              ...
              <div
                className={css.MD_MethodsTextsWrapper}
                // eslint-disable-next-line
        ...
        {!isEmpty(post.frontmatter.relativeMethodLinks[0].url) && (
          <>
            <Title className="LinkListTitle">ほかのメソッドも見る</Title>
            <div className="LinkList">
              <ul>
                {post.frontmatter.relativeMethodLinks
                  .slice(0, 3)
                  .map((relativeLink) => (
                    <li key={relativeLink.url}>
                      <Link to={`/case-studies/methods/${relativeLink.url}/`}>
                        {relativeLink.title}
                      </Link>
                    </li>
                  ))}
